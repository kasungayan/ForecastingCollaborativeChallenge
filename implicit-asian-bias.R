...
# @input: implicit_asian_bias_updated.csv
# @output: implicit_asian_bias_forecasts.csv
...
df <- read.csv("implicit_asian_bias_updated.csv", header = TRUE)

arima_model <- auto.arima(ts(df$Implicit.Asian.American.Bias, frequency = 12))
arima_forecast <- forecast(arima_model, h = 12)
arima_forecast <- as.numeric(arima_forecast$mean)


ets_model <- ets(ts(df$Implicit.Asian.American.Bias, frequency = 12))
ets_forecast <- forecast(ets_model, h = 12)
ets_forecast <- as.numeric(ets_forecast$mean)


theta_model <- thetaf(ts(df$Implicit.Asian.American.Bias, frequency = 12))
...
write.table(forecast_ensemble, "implicit_asian_bias_forecasts.txt", sep = ",", col.names = FALSE, row.names = FALSE)
