...
# @input: explicit_african_bias_updated.csv
# @output: explicit_african_bias-forecasts.csv
...
df <- read.csv("explicit_african_bias_updated.csv", header = TRUE)

arima_model <- auto.arima(ts(df$Explicit.African.American.Bias, frequency = 12))
arima_forecast <- forecast(arima_model, h = 12)
arima_forecast <- as.numeric(arima_forecast$mean)


ets_model <- ets(ts(df$Explicit.African.American.Bias, frequency = 12))
ets_forecast <- forecast(ets_model, h = 12)
ets_forecast <- as.numeric(ets_forecast$mean)


theta_model <- thetaf(ts(df$Explicit.African.American.Bias, frequency = 12))
...
write.table(forecast_ensemble, "explicit_african_bias-forecasts.txt", sep = ",", col.names = FALSE, row.names = FALSE)
